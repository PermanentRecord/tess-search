...
function getPromiseForDocument (doc) {
  return rp.get(doc.docUrl)
    .then(function (document) {
      ...
      doc.imageUrl = HOST + $('img[src*="/ImageAgent/"]').attr('src')
      doc._html = $('body').html()
...
function search (searchTerm, progressHandlerFn) {
  ...
  return Promise.using(getSession(), function (entryPageHtml) {
    ...
    return rp(HOST + link.attr('href'), { resolveWithFullResponse: true })
      ...
      .then(function (response) {
        var $ = cheerio.load(response.body)
        var titleContainingError = $(selectors.titleContainingError)
        requestsCompleted = 0
      ...
      .catch((e) => {
        // This could be an instance where only one item was returned, and the system is trying to 302 to a non-existant URL.
        if (e.statusCode === 302) {
          let redirUrl = e.response.headers.location
          ...
          //getPromiseForDocument(documentData)
        }
      })
  })
}
